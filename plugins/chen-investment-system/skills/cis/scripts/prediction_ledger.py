...
def validate_prediction(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if forbidden:
        raise ValueError(
            "public prediction ledger must not contain private portfolio fields: " + ", ".join(forbidden)
        )
...
def validate_outcome(payload: dict[str, Any], prediction: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    event["falsifier_triggered"] = _strict_optional_bool(
        payload.get("falsifier_triggered"), "falsifier_triggered"
    )
...
def record_outcome(ledger: Path, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if key in outcome_keys(events):
        raise ValueError(
            f"outcome already exists for research_id={research_id}, horizon={key[1]}"
        )
    append_event(ledger, event)
    return event
...
def materialize(ledger: Path) -> list[dict[str, Any]]:
    ...
    for research_id, prediction in predictions.items():
        outcomes = sorted(
            outcomes_by_id.get(research_id, []),
            key=lambda event: int(event.get("horizon_trading_days", 0)),
        )
        prediction["outcomes"] = outcomes
        # Backward compatibility for older single-horizon consumers/tests.
        if len(outcomes) == 1:
