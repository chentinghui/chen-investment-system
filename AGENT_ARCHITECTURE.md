...
CIS 的核心职责是**股票分析与质量控制**。量化筛选、回测和历史评估属于按需研究能力，不进入默认单股分析链。当前 CIS **不接入外部交易/回测引擎**。
...
量化研究按需分支：
CIS Research Hypothesis
  ↓
Optional Quant / Baseline Backtest
  ↓
Bias / OOS / Cost / Robustness Review
  ↓
研究证据回灌 CIS（无最终动作权）
```

## Core 与 Optional Research Tooling

CIS Core 负责分析、证据、风险、评分和交易纪律。
...
当前仓库没有独立事件驱动交易引擎。若任务需要订单撮合、复杂持仓路径、期权生命周期或其他超出 baseline evaluator 的能力，必须明确说明当前能力边界，不得把轻量 Python evaluator 冒充完整交易引擎。

Optional Extension 故障不得阻塞日常单股分析，也不得自动修改 CIS 生产权重。
...
Tactical setup 与 Research Grade 分开；高分不自动等于当前可以买。任何量化/回测结果都是第三类研究证据，不能覆盖 Evidence/Risk 质量门。
