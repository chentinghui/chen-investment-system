...
6. 涉及买卖时机、持仓复盘或具体价位时，严格执行四层结构：趋势层 → 价格层 → 成交层 → 风险层。
7. 卖出分析必须同时覆盖盈利止盈与防守止损，不得只给亏损后的卖点。
8. 将模块输出适配为 `references/io-contract.md` 的统一返回格式。
9. 解释模块冲突来自资料、假设、时间跨度还是方法差异。
10. 按所选模式生成中文输出。
11. 生成证伪条件、跟踪指标、触发动作和下一复盘日期。
