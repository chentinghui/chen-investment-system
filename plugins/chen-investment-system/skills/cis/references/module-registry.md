...
CIS 采用 **Core Analysis + External Methods + Optional Research Tooling** 分层，避免把筛选、回测、记录和绩效系统塞进日常单股分析链。
...
| CIS Scoring | 八维加权 + coverage；Research Grade 与交易赔率分离 | `production_heuristic_pending_calibration` | **Core** | 无单独动作权 |
| Market Regime Layer | profile化 risk_on / neutral / risk_off + fresh-signal filtering | `experimental` | **Core, 按需** | 无 |
| Price / Session Guard | US-equity session baseline、quote observation session、freshness | `installed` | **Core, tactical** | 质量门 |
...
- 横截面 score/forward-return sanity check：可使用 Baseline Backtest Evaluator；
- 当前没有独立事件驱动交易/回测引擎；超出 baseline 能力必须明确披露；
- 用户明确要求记录、复盘或校准：按需启用 Prediction/Evaluation Extension；
- External/Extension 故障不得阻塞 Core；
- 外部/量化研究结果不能自动改生产评分权重或发布最终买卖动作。
