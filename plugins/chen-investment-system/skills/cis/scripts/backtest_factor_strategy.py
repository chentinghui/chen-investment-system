...
def one_way_turnover(previous: dict[str, float], current: dict[str, float]) -> float:
    """One-way turnover for fully invested portfolios.

    Initial deployment from cash is 100% turnover. Later periods use
    0.5 * sum(abs(new_weight-old_weight)), bounded to [0, 1].
    """
    if not previous:
...
def run_backtest(
    ...
    for segment in ("train", "validation", "out_of_sample", "all"):
        ...
        metrics_by_segment[segment] = {
            "periods": len(subset),
            **summarize_returns(
                [p["net_return"] for p in subset],
                [p["benchmark_return"] for p in subset],
                periods_per_year,
            ),
        }
    ...
    return {
        ...
        "warning": "Input must be point-in-time and survivorship-bias-aware. Forward returns and historical universes must be created by an independent data process.",
    }
...
def main() -> int:
    ...
    parser.add_argument("--cost-bps", type=float, default=10.0, help="Cost per 100% one-way turnover")
    parser.add_argument("--periods-per-year", type=int, default=12)
    parser.add_argument("--train-end", help="Optional YYYY-MM or YYYY-MM-DD")
    parser.add_argument("--validation-end", help="Optional YYYY-MM or YYYY-MM-DD")
